    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            builder.Services.AddMassTransit(regConfig =>
            {
                ...
                regConfig.AddConsumer<ProductStockIncreasedEventConsumer>();
                regConfig.UsingRabbitMq((context, config) => {
                    ...
                    config.ReceiveEndpoint("notification-queue-available", e =>
                    {
                        e.ConfigureConsumer<StockAvailableEventConsumer>(context);
                    });

                    config.ReceiveEndpoint("notification-queue-not-available", e =>
                    {
